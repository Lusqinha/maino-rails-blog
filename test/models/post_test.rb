  ...
  class PostTest < ActiveSupport::TestCase
    ...
    test "should return posts tagged with a specific tag" do
      tagged_posts = Post.tagged_with(@tag.name)
    ...
    test "should return empty if no posts with tag" do
      tagged_posts = Post.tagged_with("non_existing_tag")
    ...
    test "should handle empty tag list" do
      @post.update_tags("")
      assert_empty @post.tags, "Tags were not removed when updating with an empty tag list"
    end
  end
